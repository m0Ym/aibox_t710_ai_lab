...
class RobotArm:
    def __init__(self):
        ...
        self.actionStand = ([500, 125, 500, 500, 500, 490, 0], [500, 500, 500])
    ...
    def transmitFrame(self, addr, data):
        ...
        if data == None:
            frame = (
                USB_FRAME_HEAD
                + addr
                + (4).to_bytes(1, byteorder="little", signed=False))
        elif len(data) > 0:
            frame = (
                USB_FRAME_HEAD
                + addr
                + (len(data) + 4).to_bytes(1, byteorder="little", signed=False)
                + data
            )
...
try:
    from lib import uart as uart_mod
except Exception:
    uart_mod = None
...
if __name__ == "__main__":
    ...
    if not capture.isOpened():
        print('[Warning] /dev/deepCamera 打开失败，尝试 /dev/video0')
        capture = cv2.VideoCapture("/dev/video0")
    if not capture.isOpened():
        print('[Warning] /dev/video0 打开失败，尝试默认摄像头 0')
