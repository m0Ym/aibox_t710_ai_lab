...
class RobotArm:
    def __init__(self):
        ...
        self.actionStand = ([500, 125, 500, 500, 500, 490, 0], [500, 500, 500, 500, 500, 500, 0])
...
class Uart:
    def __init__(self, port):
        ...
        self.len = 0
        self.recBuff = [0] * USB_FRAME_LENMAX
    ...
    def heartThread(self):
        ...
        while self.running:
            ...
            time.sleep(0.1)  # 添加延迟，避免占用过多CPU资源
    ...
    def transDataFrame(self):
        if self.recFinish:
            ...
            if self.recBuff[1] == USB_ADDR_ERRORCODE:
                self.sensors.errorCode = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                ...
                if self.recBuff[3] == bytes.fromhex("01"):
                    self.sensors.btn.btnPressA = struct.unpack("H", self.recBuff[4] + self.recBuff[5],)[0]
                    print(f"[UART DEBUG] btnA={self.sensors.btn.btnPressA}")
                elif self.recBuff[3] == bytes.fromhex("02"):
                    self.sensors.btn.btnPressB = struct.unpack("H", self.recBuff[4] + self.recBuff[5],)[0]
                    print(f"[UART DEBUG] btnB={self.sensors.btn.btnPressB}")
                elif self.recBuff[3] == bytes.fromhex("03"):
                    self.sensors.btn.btnPressC = struct.unpack("H", self.recBuff[4] + self.recBuff[5],)[0]
                    print(f"[UART DEBUG] btnC={self.sensors.btn.btnPressC}")
            elif self.recBuff[1] == USB_ADDR_INFRA:
                self.sensors.adcInfra = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                print(f"[UART DEBUG] adcInfra={self.sensors.adcInfra}")
            elif self.recBuff[1] == USB_ADDR_LASER:
                self.sensors.rangeLaser = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                print(f"[UART DEBUG] rangeLaser={self.sensors.rangeLaser}")
            elif self.recBuff[1] == USB_ADDR_THI:
                self.sensors.humi = struct.unpack("f", self.recBuff[3] + self.recBuff[4] + self.recBuff[5] + self.recBuff[6],)[0]
                self.sensors.temp = struct.unpack("f", self.recBuff[7] + self.recBuff[8] + self.recBuff[9] + self.recBuff[10],)[0]
                print(f"[UART DEBUG] humi={self.sensors.humi} temp={self.sensors.temp}")
            elif self.recBuff[1] == USB_ADDR_IMU_EULER:
                self.sensors.imu.pitch = struct.unpack("f", self.recBuff[3] + self.recBuff[4] + self.recBuff[5] + self.recBuff[6],)[0]
                self.sensors.imu.roll = struct.unpack("f", self.recBuff[7] + self.recBuff[8] + self.recBuff[9] + self.recBuff[10],)[0]
                self.sensors.imu.yaw = struct.unpack("f", self.recBuff[11] + self.recBuff[12] + self.recBuff[13] + self.recBuff[14],)[0]
                ...
                self.sensors.imu.accx = struct.unpack("h", self.recBuff[3] + self.recBuff[4],)[0]
                self.sensors.imu.accy = struct.unpack("h", self.recBuff[5] + self.recBuff[6],)[0]
                self.sensors.imu.accz = struct.unpack("h", self.recBuff[7] + self.recBuff[8],)[0]
                self.sensors.imu.gyrox = struct.unpack("h", self.recBuff[9] + self.recBuff[10],)[0]
                self.sensors.imu.gyroy = struct.unpack("h", self.recBuff[11] + self.recBuff[12],)[0]
                self.sensors.imu.gyroz = struct.unpack("h", self.recBuff[13] + self.recBuff[14],)[0]
                ...
                self.sensors.co2 = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                if self.sensors.co2 < 400:
                    self.sensors.co2 = 400
                elif self.sensors.co2 > 999:
                    self.sensors.co2 = 999
                self.sensors.tvoc = struct.unpack("H", self.recBuff[5] + self.recBuff[6],)[0]
                ...
                self.sensors.ges.gesAreaDet = struct.unpack("H", self.recBuff[5] + self.recBuff[6],)[0]
                ...
                self.sensors.illume = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                print(f"[UART DEBUG] illume={self.sensors.illume}")
            elif self.recBuff[1] == USB_ADDR_MIC:
                self.sensors.adcMic = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                print(f"[UART DEBUG] adcMic={self.sensors.adcMic}")
            elif self.recBuff[1] == USB_ADDR_RADAR:
                self.sensors.rangeRadar = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                print(f"[UART DEBUG] rangeRadar={self.sensors.rangeRadar}")
            elif self.recBuff[1] == USB_ADDR_KEYBOARD:
                self.sensors.keyCount = struct.unpack("I", self.recBuff[3] + self.recBuff[4] + self.recBuff[5] + self.recBuff[6],)[0]
                print(f"[UART DEBUG] keyCount={self.sensors.keyCount}")
            elif self.recBuff[1] == USB_ADDR_KNOB_NUM:
                self.sensors.knob.count = struct.unpack("h", self.recBuff[3] + self.recBuff[4],)[0]
                print(f"[UART DEBUG] knob.count={self.sensors.knob.count}")
            elif self.recBuff[1] == USB_ADDR_KNOB_BTN:
                self.sensors.knob.btnPress = struct.unpack("H", self.recBuff[3] + self.recBuff[4],)[0]
                self.sensors.knob.count = 0
                print(f"[UART DEBUG] knob.btnPress={self.sensors.knob.btnPress}")
            elif self.recBuff[1] == USB_ADDR_TRANS:
                self.sensors.transSpeed = struct.unpack("f", self.recBuff[3] + self.recBuff[4] + self.recBuff[5] + self.recBuff[6],)[0]
    ...
    def transmitFrame(self, addr, data):
        ...
        if data is None:
            ...
            frame = (
                USB_FRAME_HEAD
                + addr
                + frame_length.to_bytes(1, byteorder="big", signed=False))
        else:
        ...
        # 添加校验和和额外的00字节
        frame = frame + check.to_bytes(1, byteorder="big", signed=False) + bytes.fromhex("00")
...
if __name__ == "__main__":
    ...
    try:
        ...
        uart.startHeart()  # 开启心跳线程，维持与下位机的连接
    except Exception as e:
