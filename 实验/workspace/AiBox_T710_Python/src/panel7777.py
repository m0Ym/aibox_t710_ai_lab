...
def initialize_robot_arm():
    try:
        ...
        time.sleep(1.0)
        print("Robot arm reset to initial position")
        return True
    except Exception as e:
        print(f"Robot arm init failed: {e}")
        return False


def initialize_sensors():
    try:
        # 传感器初始化 - 实际传感器数据通过UART从下位机获取
        # 这里可以添加一些传感器状态检查，但主要是确保UART通信正常
        print("Sensors initialized via UART communication")
...
def cleanup_system():
    """系统级清理"""
    print("[CLEANUP] 开始系统清理...")
    
    # 对于传感器和机械臂，通过UART发送复位命令
    try:
        # 如果uart对象存在，发送复位命令
        global uart
        if 'uart' in globals() and uart:
            try:
                # 复位机械臂到安全位置
                uart.robotArmMoveit(uart.robotArm.Movement.reset)
                time.sleep(0.5)
            except:
                pass  # 如果UART不可用，跳过此步骤
    except:
        pass
    
    # 需要清理的设备列表
    devices_to_clean = ['/dev/ttyUSB0', '/dev/video0', '/dev/deepCamera']
    
    for device in devices_to_clean:
        try:
            # 检查设备是否存在
            if os.path.exists(device):
                print(f"[CLEANUP] 释放设备: {device}")
                # 先尝试使用fuser -k杀死占用进程
                result = subprocess.run(['fuser', '-k', device], timeout=5, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
                time.sleep(0.3)
                # 再次检查进程是否还存在，如果存在则使用更强制的方式
                result = subprocess.run(['fuser', '-k', '-TERM', device], timeout=5, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
                time.sleep(0.3)
            else:
                print(f"[CLEANUP] 设备不存在，跳过: {device}")
        except subprocess.TimeoutExpired:
            print(f"[CLEANUP] 释放设备超时: {device}")
        except Exception as e:
            print(f"[CLEANUP] 释放设备失败 {device}: {e}")
    
    time.sleep(0.5)
    print("[CLEANUP] 系统清理完成")


# 全局变量用于保存UART和摄像头对象，以便在程序异常退出时清理
uart_instance = None
capture_instance = None


def global_cleanup():
    """全局清理函数，用于atexit和异常退出时的资源清理"""
    global uart_instance, capture_instance
    
    print("[GLOBAL_CLEANUP] 执行全局清理...")
    
    # 关闭串口
    if uart_instance:
        try:
            uart_instance.setTransSpeed(0.0)
            uart_instance.setRelaySwitch(False, False, False)
            uart_instance.robotArmMoveit(uart_instance.robotArm.Movement.reset)
            time.sleep(0.5)
            uart_instance.running = False
            if hasattr(uart_instance, 'thread') and uart_instance.thread.is_alive():
                uart_instance.thread.join(timeout=1.0)
            if hasattr(uart_instance, 'threadHeart') and uart_instance.threadHeart and uart_instance.threadHeart.is_alive():
                uart_instance.threadHeart.join(timeout=1.0)
            # 显式调用close方法来确保串口被关闭
            uart_instance.close()
            print("[GLOBAL_CLEANUP] UART已关闭")
        except Exception as e:
            print(f"[GLOBAL_CLEANUP] UART清理失败: {e}")
    
    # 释放摄像头
    if capture_instance:
        try:
            capture_instance.release()
            print("[GLOBAL_CLEANUP] 摄像头已释放")
        except Exception as e:
            print(f"[GLOBAL_CLEANUP] 摄像头释放失败: {e}")
    
    # 关闭所有OpenCV窗口
    try:
        cv2.destroyAllWindows()
        print("[GLOBAL_CLEANUP] OpenCV窗口已关闭")
    except Exception as e:
        print(f"[GLOBAL_CLEANUP] OpenCV窗口关闭失败: {e}")
    
    # 系统级清理
    cleanup_system()
    print("[GLOBAL_CLEANUP] 全局清理完成")
...
class Panel:
    ...
    def __init__(self) -> None:
        ...
        # 机械臂动作状态管理（防止跟踪与初始化冲突）
        self.arm_tracking_enabled = False  # 默认禁用跟踪，初始化完成后启用
        self.arm_action_in_progress = False  # 机械臂是否正在执行动作
        self.arm_busy_until = 0.0  # 机械臂忙碌结束时间
    ...
    def process_face_landmark(self, frame):
        ...
        try:
            # 检查机械臂状态：如果跟踪被禁用或正在执行动作，跳过跟踪
            if not self.arm_tracking_enabled or self.arm_action_in_progress:
                return frame_with_landmark
            
            # 检查机械臂是否忙碌
            if time.time() < self.arm_busy_until:
                return frame_with_landmark
    ...
    def wait_for_arm_completion(self, timeout=10.0):
        """等待机械臂动作完成。

        基于 uart.robotArm 的 timeStart 和 timeSpent 计算动作完成时间。
        如果超时或 uart 不可用，则返回 False。
        ...
        Returns:
            bool: 动作是否成功完成
        """
        if 'uart' not in globals():
            print("[DEBUG] wait_for_arm_completion: uart not available")
            return False
        ...
        while time.time() < end_time:
            try:
                robot_arm = uart.robotArm
                if robot_arm.action:
                    # 计算预期完成时间
                    expected_end = robot_arm.timeStart + (robot_arm.timeSpent / 1000.0)
                    current_time = time.time()

                    if current_time >= expected_end:
                        print(f"[DEBUG] wait_for_arm_completion: 动作已完成 (timeSpent={robot_arm.timeSpent}ms)")
                        break

                    remaining = expected_end - current_time
                    print(f"[DEBUG] wait_for_arm_completion: 等待中... 剩余 {remaining:.2f}秒")
                    time.sleep(min(0.1, remaining))
                else:
                    print(f"[DEBUG] wait_for_arm_completion: 动作未开始或已完成")
                    break
            except Exception as e:
                print(f"[DEBUG] wait_for_arm_completion: 检查状态失败: {e}")
                time.sleep(0.1)

        # 更新机械臂忙碌时间（用于跟踪协调）
        self.arm_busy_until = time.time() + 0.5
        print(f"[DEBUG] wait_for_arm_completion: 设置 arm_busy_until={self.arm_busy_until}")

        elapsed = time.time() - start_time
        if elapsed >= timeout:
            print(f"[Warning] wait_for_arm_completion: 超时 ({timeout}秒)")
            return False
        else:
            print(f"[DEBUG] wait_for_arm_completion: 完成 (耗时 {elapsed:.2f}秒)")
            return True
    ...
    def init_robot_arm(self, action=None, wait_for_uart=True):
        ...
        # 禁用跟踪，标记动作进行中
        self.arm_tracking_enabled = False
        self.arm_action_in_progress = True
        ...
        if 'uart' in globals() and action is not None:
            try:
                ...
                if getattr(uart, 'running', False):
                    ...
                    # 等待机械臂动作完成
                    self.wait_for_arm_completion(timeout=10.0)
                elif wait_for_uart:
        ...
        if success:
            ...
            # 初始化完成后，等待一段时间再启用跟踪
            time.sleep(1.0)
            self.arm_tracking_enabled = True
            self.arm_action_in_progress = False
            print(f"[DEBUG] 机械臂跟踪已启用")
        else:
...
if __name__ == "__main__":
    ...
    # uart_instance 已在文件顶部声明为全局变量，此处直接赋值即可，无需再次 global 声明
    uart_instance = uart  # 保存UART实例用于异常退出时清理
    ...
    # 初始化机械臂和传感器
    if not initialize_robot_arm():
        print("Robot arm init failed. Exiting.")
        exit(1)
    if not initialize_sensors():
        print("Sensors init failed. Exiting.")
        exit(1)

    # 开启摄像头，若 /dev/deepCamera 无法打开则回退到 /dev/video0 再回退到默认设备0
    capture = cv2.VideoCapture("/dev/deepCamera")
    # capture_instance 已在文件顶部声明为全局变量，此处仅使用，无需再次 global 声明
    capture_instance = capture  # 保存摄像头实例用于异常退出时清理
