...
def CalibrationCamera(
    ...
    try:
        ...
        sys.exit(0)
    with open(res_path('res','calibration','assessment.txt'), "w") as f:
    ...
    for i in range(len(objectPoints)):
        ...
        with open(res_path('res','calibration','assessment.txt'), "a") as f:
...
if __name__ == "__main__":
    inputFolder = res_path('res','calibration','temp')  # 输入图像文件夹
    outputFolder = res_path('res','calibration') + os.sep  # 输出图像文件夹
    ...
    tree.write(
        res_path('res','calibration','calibration.xml'),
        encoding="utf-8",
